  ...
  return (
    <section className="cta" id="contacto">
      <div className="wrap cta-grid">
        ...
        <form>
          <div className="field"><label htmlFor="n">Nombre *</label><input id="n" required /></div>
          <div className="row2"><div className="field"><label htmlFor="e">Email *</label><input id="e" type="email" required /></div><div className="field"><label htmlFor="t">Teléfono *</label><input id="t" required /></div></div>
          <div className="field"><label htmlFor="emp">Empresa *</label><input id="emp" required /></div>
          <div className="field"><label htmlFor="obj">¿En qué te podemos ayudar? *</label><select id="obj" required><option value="">Seleccionar objetivo</option><option>Financiamiento</option><option>Inversión y liquidez de la empresa</option><option>Portfolio personal (socios)</option><option>Medios de pago</option><option>Automatización con IA</option><option>Quiero ser referenciador (Agente Productor)</option><option>Consulta general</option></select></div>
          <div className="field"><label htmlFor="m">Mensaje (opcional)</label><textarea id="m" /></div>
          <button className="btn btn-primary" type="submit">Agendar una llamada</button>
          <p className="legal">Al enviar este formulario, aceptás nuestra <a href="#">Política de Privacidad</a>. ¿Preferís WhatsApp? Respondemos dentro de 1 día hábil.</p>
        </form>
      </div>
    </section>
  );
